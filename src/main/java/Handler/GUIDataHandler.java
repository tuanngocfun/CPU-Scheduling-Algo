...
// /**
//  * Handles the processing of GUI input data for scheduling simulations.
//  * @version 1.1
//  */
// public class GUIDataHandler {
...
//             Scheduler scheduler = initializeScheduler(input);
//             Set<CycleComponent> components = initializeComponents();
            
//             processScheduleTable(tableModel, scheduler.getScheduleTable(), components);
            
//             RunResult runResult = scheduler.run();
//             return generateGUIProcessedData(scheduler.getScheduleTable(), components, runResult);
//         } catch (IllegalArgumentException e) {
//             return null;
//         }
//     }

//     private Scheduler initializeScheduler(GUIInputData input) {
//         Scheduler scheduler = new Scheduler();
//         scheduler.setCPUAlgorithm(input.getCPUSchedulingAlgorithm());
//         scheduler.setResourceAlgorithm(input.getResourceSchedulingAlgorithm());
//         return scheduler;
...
//     private Set<CycleComponent> initializeComponents() {
//         Set<CycleComponent> components = new HashSet<>();
//         CPU cpu = new CPU(new GanttChart());
//         components.add(cpu);
//         return components;
...
//     private void processScheduleTable(ScheduleTableModel tableModel, ScheduleTable table, Set<CycleComponent> components) {
//         for (int i = 0; i < tableModel.getProcessCount(); i++) {
//             Process process = new Process(i + 1);
//             SchedulingInfo info = createSchedulingInfo(tableModel, i);
//             table.setSchedule(process, info);
...
//     private SchedulingInfo createSchedulingInfo(ScheduleTableModel tableModel, int processIndex) {
//         int RQArrivalTime = Integer.parseInt(tableModel.getValueAt(processIndex, tableModel.getRQArrivalTimeColumnIndex()).toString());
//         int systemArrivalTime = Integer.parseInt(tableModel.getValueAt(processIndex, tableModel.getSystemArrivalTimeColumnIndex()).toString());
...
//             Object cpuBurstValue = tableModel.getValueAt(processIndex, CPUBurstColumnIndex);
//             Object ioBurstValue = tableModel.getValueAt(processIndex, IOBurstColumnIndex);

//             String cpuBurstInput = (cpuBurstValue != null) ? cpuBurstValue.toString() : "";
//             String ioBurstInput = (ioBurstValue != null) ? ioBurstValue.toString() : "";

//             CPUBurst cpuBurst = parseCPUBurst(cpuBurstInput, info);
//             IOBurst ioBurst = parseIOBurst(ioBurstInput, components);

//             info.addCycle(currentCycle, new Cycle(cpuBurst, ioBurst));
//         }
//     }

//     private CPUBurst parseCPUBurst(String input, SchedulingInfo info) {
//         return (!input.isEmpty()) ? new CPUBurst(new CPU(), Integer.parseInt(input)) : null;
//     }

//     private IOBurst parseIOBurst(String input, Set<CycleComponent> components) {
//         if (input.isEmpty()) {
//             return null;
//         }
...
//     private GUIProcessedData generateGUIProcessedData(ScheduleTable table, Set<CycleComponent> components, RunResult runResult) {
//         Map<Process, RunningInterval> processRunningIntervalMap = Arrays.stream(table.getProcesses())
//             .collect(Collectors.toMap(
//                 process -> process,
//                 process -> generateRunningIntervalForProcess(process, components)
//             ));
//         return new GUIProcessedData(runResult, processRunningIntervalMap);
//     }

//     private RunningInterval generateRunningIntervalForProcess(Process process, Set<CycleComponent> components) {
//         RunningInterval runningInterval = new RunningInterval();
//         components.forEach(component -> {
